...
import type { IncomeAiSummary } from '@/lib/finance/types'
import { cn } from '@/lib/utils'

interface AnalysisResult {
  summary: IncomeAiSummary
  model: string
  created_at: string
}

export default function IncomeAiPanel({
  periodStart,
  periodEnd,
}: {
  periodStart: string
  ...
  const runAnalysis = async () => {
    ...
    setError(null)
    try {
      const res = await fetch('/api/finance/incomes-ai-analysis', {
        method: 'POST',
  ...
  if (!analysis) {
    return (
      <div className="flex flex-col items-center justify-center py-10 gap-4 text-center min-h-[280px]">
        ...
        <div>
          <p className="text-sm font-semibold text-slate-900">Análisis inteligente de ingresos</p>
          <p className="text-xs text-slate-500 mt-1.5 max-w-xs leading-relaxed">
            Conciliación banco-factura, clientes con bajo cobro, MRR y brechas de trazabilidad.
            El IVA solo se considera si hay facturas vinculadas.
          </p>
        </div>
        <Button onClick={runAnalysis} disabled={loading} size="sm" className="gap-2">
          {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Search className="h-4 w-4" />}
          Analizar ingresos con IA
        </Button>
  ...
  const score = s.salud_cobros_0_100 ?? 0
    ...
    <div className="space-y-4">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="text-center shrink-0 rounded-lg border border-slate-100 bg-slate-50 px-3 py-2">
          ...
          <p className="text-[9px] text-slate-400 uppercase tracking-wide">Salud cobros</p>
        </div>
      </div>
      ...
      {s.por_cliente?.length > 0 && (
        <div className="space-y-1.5 border-t border-slate-100 pt-3">
          <p className="text-[10px] uppercase tracking-wide text-slate-400">Por cliente</p>
          {s.por_cliente.map((c) => (
