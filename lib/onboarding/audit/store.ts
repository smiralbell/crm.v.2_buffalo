...
export async function getAuditByLeadId(leadId: number): Promise<ProjectAudit | null> {
  try {
    ...
    // Compat si aún no se ha ejecutado ALTER v2
    const msg = e instanceof Error ? e.message : ''
    if (!/questions|active_question_id|progress|gaps/i.test(msg)) throw e
    const { rows } = await query<Row>(
...
export async function saveAudit(audit: ProjectAudit): Promise<ProjectAudit> {
  try {
    ...
    const msg = e instanceof Error ? e.message : ''
    if (!/questions|active_question_id|progress|gaps|started_at/i.test(msg)) throw e
    const { rows } = await query<Row>(
      `UPDATE project_audits SET
         project_types = $2,
         active_mode = $3,
